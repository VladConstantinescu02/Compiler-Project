...
// After typeBase, check: is the next token a { ?
Token *midTk = crtTk;
if (midTk && midTk->code == LACC) {
    crtTk = startTk;  // rewind everything
    return 0;
}
...
if (consume(ID)) {
    ...
    while (consume(COMMA)) {
        if (!consume(ID)) tkerr(crtTk, "missing ID after comma in declaration");
        arrayDecl();
    }
...
if (consume(STRUCT)) {
    if (consume(ID)) {
        printf("LOOKING FOR LACC, crtTk is: %dn", crtTk->code);
        if (consume(LACC)) {
            while (declVar()) {
