...
int exprPrimary() {
    ...
    if (consume(ID)) {
        if (consume(LPAR)) {
            if (!consume(RPAR)) {
                if (!expr()) tkerr(crtTk, "missing expression in function call");
                while (consume(COMMA)) {
...
int exprPostfix() {
    ...
    while (1) {
        if (consume(LBRACKET)) {
            if (!expr()) tkerr(crtTk, "missing expression in array access");
            if (!consume(RBRACKET)) tkerr(crtTk, "missing ] after array index");
        } else if (consume(DOT)) {
...
int exprUnary() {
    if (consume(SUB) || consume(NOT)) {
        if (!exprUnary()) tkerr(crtTk, "missing operand for unary operator");
        return 1;
    }
    return exprPostfix();
}
...
int exprCast() {
    ...
    if (consume(LPAR)) {
        if (typeName()) {
            if (!consume(RPAR)) tkerr(crtTk, "missing ) after type name");
...
int exprMul() {
    ...
    while (consume(MUL) || consume(DIV)) {
        if (!exprCast()) tkerr(crtTk, "missing right-hand operand for * or /");
    }
    return 1;
}
...
int exprAdd() {
    ...
    while (consume(ADD) || consume(SUB)) {
        if (!exprMul()) tkerr(crtTk, "missing right-hand operand for + or -");
    }
    return 1;
}
...
int exprEq() {
    ...
    while (consume(EQUAL) || consume(NOTEQ)) {
        if (!exprRel()) tkerr(crtTk, "missing right-hand operand for == or !=");
    }
    return 1;
}
...
int exprOr() {
    if (!exprAnd()) return 0;
    while (consume(OR)) {
        if (!exprAnd()) tkerr(crtTk, "missing right-hand operand for ||");
    }
    return 1;
}
...
int stm() {
    ...
    if (consume(IF)) {
        ...
        if (!expr()) tkerr(crtTk, "missing condition in if");
        if (!consume(RPAR)) tkerr(crtTk, "missing ) after if condition");
        if (!stm()) tkerr(crtTk, "missing statement after if");
        if (consume(ELSE) && !stm()) tkerr(crtTk, "missing statement after else");
        return 1;
    }
    if (consume(WHILE)) {
        if (!consume(LPAR)) tkerr(crtTk, "missing ( after while");
        if (!expr()) tkerr(crtTk, "missing condition in while");
    ...
    if (consume(SEMICOLON)) return 1; // empty statement
...
int declFunc() {
    if (!(typeBase() || consume(VOID))) return 0;
    consume(MUL);
    if (!consume(ID)) tkerr(crtTk, "missing function name");
    if (!consume(LPAR)) tkerr(crtTk, "missing ( in function");
    if (funcArg()) {
        while (consume(COMMA)) {
            if (!funcArg()) tkerr(crtTk, "missing function argument");
        }
    }
    if (!consume(RPAR)) tkerr(crtTk, "missing ) in function");
    if (!stmCompound()) tkerr(crtTk, "missing function body");
    return 1;
}

int typeBase() {
    if (consume(INT) || consume(DOUBLE) || consume(CHAR)) return 1;
    if (consume(STRUCT)) {
...
int funcArg() {
    if (!typeBase()) return 0;
    if (!consume(ID)) tkerr(crtTk, "missing parameter name");
    arrayDecl();
    return 1;
}
